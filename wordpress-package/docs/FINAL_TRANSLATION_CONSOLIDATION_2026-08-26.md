...
| Portail marchand arabe | 390 × 844 px | Route publique de connexion | Rendu local RTL contrôlé ; l’accès authentifié n’a pas été rejoué faute de credentials de recette confirmés |
...
## Portail marchand et limites d’authentification
...
La présente matrice a validé la surface publique et la page de connexion arabe. Elle **ne prétend pas valider une session authentifiée** : aucun identifiant ou mot de passe n’est inclus dans le dépôt public, et aucun secret ne doit être inventé ou publié. La prochaine recette doit utiliser un compte de staging fourni séparément, puis contrôler l’ajout d’un produit de test, la modification du prix/stock, l’ajout d’une catégorie ou d’options, l’aperçu de média, la consultation d’une commande de recette et le retour arrière.

## Ce qui reste ouvert

La traduction gratuite est techniquement en place, mais une dernière passe éditoriale doit encore vérifier chaque titre, description, catégorie, attribut, option, message de panier et message de checkout en arabe. Les chaînes dynamiques doivent être sélectionnées dans TranslatePress après l’action WooCommerce qui les rend visibles, notamment la sélection d’une variation, l’ouverture du panier et l’affichage des notices de checkout [2].
