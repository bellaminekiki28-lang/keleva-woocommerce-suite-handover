...
async function activeConnection(storeId: number) {
  ...
  if (
    ...
    throw new Error("Store connection is not active");
  return {
    baseUrl: store.baseUrl,
    consumerKey: decryptConnectionSecret(connection.encryptedConsumerKey),
    consumerSecret: decryptConnectionSecret(connection.encryptedConsumerSecret),
  };
}
...
export async function wooRequest<T>(
  ...
  const response = await fetch(
    `${connection.baseUrl}/wp-json/wc/v3/${path.replace(/^\//, "")}`,
    {
      ...init,
      headers: {
...
export async function listWooOrders(
  ...
  const { data } = await wooRequest<WooOrder[]>(
    storeId,
...
export async function listWooProducts(
  ...
  const { data } = await wooRequest<WooProduct[]>(
    storeId,
    `products?per_page=${limit}&orderby=modified&order=desc`
  );
  return data;
}
...
export async function updateWooOrder(
  ...
  const { data } = await wooRequest<WooOrder>(storeId, `orders/${orderId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function findWooProductBySku(
  storeId: number,
  sku: string
): Promise<WooProduct | null> {
  const { data } = await wooRequest<WooProduct[]>(
    storeId,
    `products?sku=${encodeURIComponent(sku)}&per_page=2`
  );
  if (data.length > 1)
    throw new Error("WooCommerce returned duplicate SKU results");
  return data[0] ?? null;
}
...
export async function createWooProduct(
  ...
  const { data } = await wooRequest<WooProduct>(storeId, "products", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function deleteWooProduct(
  ...
  await wooRequest<WooProduct>(storeId, `products/${productId}?force=true`, {
    method: "DELETE",
  });
}
